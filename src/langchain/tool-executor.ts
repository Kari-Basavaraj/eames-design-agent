...
import { callLlm, getToolSelectionModel } from '../model/llm.js';
import { ToolContextManager } from './context.js';
import { getToolSelectionSystemPrompt, buildToolSelectionPrompt } from '../shared/prompts.js';
import type { Task, ToolCallStatus, Understanding } from '../types/state.js';
...
export interface ToolExecutorOptions {
  ...
  /** Main model from Agent - tool selection uses provider-matching small model */
  model: string;
}
...
export class ToolExecutor {
  ...
  /**
   * Selects tools for a task using a small model (matches user's provider).
  ...
  async selectTools(
    ...
    const smallModel = getToolSelectionModel(this.model);
