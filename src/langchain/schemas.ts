...
export const UnderstandingSchema = z.object({
  ...
  needsClarification: z.boolean()
    .default(false)
    .describe('True if the query is vague and we should ask 2-4 clarifying questions before proceeding'),
  clarifyingQuestions: z.array(z.string())
    .default([])
    .describe('2-4 short questions to ask the user when needsClarification is true (e.g., "What age range?", "Web or mobile?")'),
});
...
export const ReflectionSchema = z.object({
  ...
  proceedQuestion: z.string()
    .default('')
    .describe('When incomplete: friendly question to ask user, e.g. "Should I proceed with implementing the code?" (empty if complete)'),
  completedSummary: z.string()
    .default('')
    .describe('When incomplete: brief summary of what was done, e.g. "PRD and research" (empty if complete)'),
});
