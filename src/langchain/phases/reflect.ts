...
import { getReflectSystemPrompt, buildReflectUserPrompt } from '../../shared/prompts.js';
import type { ReflectInput, ReflectionResult, Plan, TaskResult } from '../../types/state.js';
...
export class ReflectPhase {
  ...
  async run(input: ReflectInput): Promise<ReflectionResult> {
    ...
    return {
      ...
      proceedQuestion: result.proceedQuestion ?? '',
      completedSummary: result.completedSummary ?? '',
    };
  }
