...
2. **Calculate Metrics**
   ...
   **File Sync Check:**
   Cross-reference validation between all 3 files:
   - Pattern refs (PAT-XXX) in backlog → must exist in IDEA_PATTERNS.md
   - Backlog refs (FI-XXX) in patterns → must exist in FEATURE_IDEAS_BACKLOG.md
   - Inbox items marked "✅ Processed" → should have corresponding FI-XXX
...
3. **Display Dashboard**
   ...
   ║   OR                                                      ║
   ║   ⚠️ X broken refs found (run with --fix to repair)       ║
   ╚═══════════════════════════════════════════════════════════╝
   ```

   **File Sync Details (if issues found):**
   ```
   FILE SYNC CHECK:
   ✅ FI-001 → PAT-001 exists in IDEA_PATTERNS.md
   ✅ PAT-001 → FI-001, FI-002 exist in backlog
   ⚠️ IN-003 marked "Processed" but no matching FI-XXX found
   ⚠️ FI-002 references PAT-003 which doesn't exist
   ```
...
4. **Recommendations**
   ...
   - "🔗 X broken file refs - fix manually or re-run analysis"
